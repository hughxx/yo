...
class RuleDialog(QDialog):
    def __init__(self, rule=None, parent=None):
        ...
        self.body_keywords_edit.setPlaceholderText('逗号分隔，匹配邮件正文（本地 Outlook 搜索）')
...
class SettingsDialog(QDialog):
    ...
    def _make_basic(self):
        ...
        self._file_server = QLineEdit(self._s.get('fileServerUrl', ''))
        self._file_server.setPlaceholderText('http://host:port')
        lay.addRow('File server URL:', self._file_server)

        self._image_public_base = QLineEdit(self._s.get('imagePublicBase', ''))
        self._image_public_base.setPlaceholderText('https://public-host')
        lay.addRow('Image public base:', self._image_public_base)

        self._llm_base_url = QLineEdit(self._s.get('llmBaseUrl', ''))
        self._llm_base_url.setPlaceholderText('https://host/v1')
        lay.addRow('LLM base URL:', self._llm_base_url)

        self._llm_api_key = QLineEdit(self._s.get('llmApiKey', ''))
        self._llm_api_key.setEchoMode(QLineEdit.Password)
        lay.addRow('LLM API key:', self._llm_api_key)

        self._llm_model = QLineEdit(self._s.get('llmModel', ''))
        lay.addRow('LLM model:', self._llm_model)
    ...
    def _on_save(self):
        ...
        self._s['fileServerUrl']       = self._file_server.text().strip()
        self._s['imagePublicBase']     = self._image_public_base.text().strip()
        self._s['llmBaseUrl']          = self._llm_base_url.text().strip()
        self._s['llmApiKey']           = self._llm_api_key.text().strip()
        self._s['llmModel']            = self._llm_model.text().strip()
...
class SetupDialog(QDialog):
    ...
    def __init__(self, settings: dict, parent=None):
        ...
        self._server_combo.setCurrentText(_url_to_display(self._s.get('backendUrl', '')) or '')
    ...
    def _load_namespaces(self):
        ...
        if not url:
            ...
            self._ns_combo.addItem('local', 'local')
            return
        backend.set_base(url)
    ...
    def _confirm(self):
        ...
        user_id = self._confirmed_uid or self._user_id.text().strip()

        if not user_id:
            QMessageBox.warning(self, 'Error', 'Please enter user id')
            return

        if False and not url:
            QMessageBox.warning(self, '错误', '请选择或输入服务器地址')
            return
        if False and not self._confirmed_uid:
