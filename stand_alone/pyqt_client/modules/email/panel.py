...
class EmailPanel(QWidget):
    ...
    def _is_configured(self) -> bool:
        s = self._settings
        return bool(s.get('userId') and s.get('namespace'))
    ...
    def _import_msg_batch(self, paths, offset, success, failed):
        ...
        img_api  = self._settings.get('fileServerUrl', '') or self._settings.get('backendUrl', '')
            ...
            for p in chunk:
                try:
                    ...
                    backend.receive_email({
                        ...
                        'MarkdownBody':      local_archive.html_to_markdown(item.get('html_body', '')),
